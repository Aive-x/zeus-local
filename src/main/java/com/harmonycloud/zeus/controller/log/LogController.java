...
public class LogController {
     ...
     * @date 2021/6/17 5:48 下午
     */
    @ApiOperation(value = "导出查询日志", notes = "导出查询日志")
    @ResponseBody
    @RequestMapping(method = RequestMethod.POST)
    public ActionReturnUtil queryLog(@PathVariable("clusterId") String clusterId,
     ...
     * @date 2021/6/21 5:05 下午
     */
    @ApiOperation(value = "从es获取pod的日志文件列表", notes = "从es获取pod的日志文件列表")
    @ResponseBody
    @RequestMapping(value = "/filenames", method = RequestMethod.GET)
    public ActionReturnUtil listLogFilenames(@PathVariable("clusterId") String clusterId,
        @PathVariable("namespace") String namespace,
        @PathVariable("middlewareName") String middlewareName,
        @ModelAttribute LogQueryDto logQueryDto) {
        try {
            ...
            logger.error("获取pod的日志文件列表失败：deploymentName:{}", middlewareName, e);
