...
|   |── main/                                               # 源码目录
|       |── java/                                           # 源码
|           |── com.harmonycloud.caas.middleware/           # 基础包名
|               |── annotation/                             # 自定义注解
|               |── bean/                                   # 数据库实体类
|               |── config/                                 # 自动装配
...
|                   |── minio/                              # 封装minio的接口调用
|               |── interceptor/                            # 拦截器
|                   |── CaasMiddlewareInterceptor.java      # 拦截器
...
|               |── CaasMiddlewareApplication.java          # SpringBoot启动类
|       |── resource/                                       # 资源
|           |── config/                                     # 配置文件目录
|           |── mappiing/                                   # xml文件目录
...
k8s:
  ...
  token:
    ...
    path: /Users/dengyulong/Desktop/upload/token # change me
  kubeconfig:
    # debug时填写本地的kube config文件路径，有默认值，仅开发使用
    path: /Users/dengyulong/Desktop/upload
system:
  upload:
    # debug时填写自己本地的一个可访问的目录，仅开发使用，默认值：/usr/local/caas-middleware-pv/upload
    path: /Users/dengyulong/Desktop/upload
```
其中：
